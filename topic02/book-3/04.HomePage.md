...
~~~
Object destructuring is used on the props object ( {movies} ) for illustration purposes.

Ultimately this page will be plugged into a routing component. In the meantime we will statically link it to a top-level component that feeds it an array of movies. Create `src/index.js` and code it as follows:
~~~
import React from "react";
...
The server provides live reloading. To illustrate, in `src/index.html` change the movies const to:
~~~
